...
for i in range(2,len(columns)-1,2):
    ...
    df_sivic[title2] = np.where((df_sivic[title] == 4.0) & (df_sivic[title_bis] != 4.0), 1, 0)
...
for i in range(2,len(columns)-new_columns,2):
    ...
    df_sivic[title2] = np.where((df_sivic[title] == 4.0) & (df_sivic[title_bis] == 1.0), 1, 0)
...
for i in range(2,len(columns)-new_columns,2):
    ...
    df_sivic[title2] = np.where((df_sivic[title] == 1.0) & (df_sivic[title_bis] == 4.0), 1, 0)
...
for i in range(2,len(columns)-new_columns,2):
    ...
    df_sivic[title2] = np.where((df_sivic[title] == 5.0) & (df_sivic[title_bis] == 4.0), 1, 0)
...
for i in df_sivic.index:
   ...
   if i[0] == 'H':
       hospit += [df_sivic[i]]
   if i[:11]=='Entrée_réaH':
       entree_SSR += [df_sivic[i]]
   if i[:13]=='Entrée_réa_HC':
       entree_SSR_HC += [df_sivic[i]]
   if i[:13]=='Entrée_réa_SU':
       entree_SSR_SU += [df_sivic[i]]
   if i[:14]=='Entrée_réa_SSR':
       entree_SSR_rea += [df_sivic[i]]
   if i[:11]=='Sortie_réaH':
       sortie_SSR += [df_sivic[i]]
   if i[:13]=='Sortie_réa_HC':
       sortie_SSR_HC += [df_sivic[i]]
   if i[:13]=='Sortie_réa_dc':
       sortie_SSR_dc += [df_sivic[i]]
   if i[:14]=='Sortie_réa_SSR':
       sortie_SSR_rea += [df_sivic[i]]
   if i[:14]=='Sortie_réa_RAD':
       sortie_SSR_RAD += [df_sivic[i]]



data_SSR={
    'Entrées en SSR':entree_SSR,
    'Sortie de SSR': sortie_SSR
        }

data_entree={'Entrées en SSR':entree_SSR,
    'dont depuis HC': entree_SSR_HC,
    'dont depuis rea': entree_SSR_rea,
             'dont depuis Soin aux urgences':entree_SSR_SU
             }

data_sortie={
    'Sorties de SSR': sortie_SSR,
    'dont décès': sortie_SSR_dc,
    'dont vers HC':sortie_SSR_HC,
    'dont RAD': sortie_SSR_RAD,
    'dont vers réa': sortie_SSR_rea
             }

def plot_dc(data_dc, X):

    df_final=pd.DataFrame(data_dc)
    df_final["date"]=X
    ...
    plt.savefig('data/figures/plot_décès.png')
    plt.show()

def plot_SSR(data_SSR, X):
    df_final=pd.DataFrame(data_SSR)
    df_final["date"]=X
    df_final=df_final.set_index("date")
    #my_colors = [(0, 0, 1), (0, 1, 0)] * len(X)
    ...
    for p in ax.patches:
        ...
        ax.annotate(val, ((b.x0 + b.x1)/2 , int(b.y1)), ha='center', va='bottom')
    plt.xlabel('Date',rotation=0)
    plt.savefig('data/figures/plot_SSR.png')
    plt.show()

def plot_SSR_sortie(data_SSR, X):
    df_final=pd.DataFrame(data_SSR)
    df_final["date"]=X
    df_final=df_final.set_index("date")
    ...
    for p in ax.patches:
        ...
        ax.annotate(val, ((b.x0 + b.x1)/2 , int(b.y1)), ha='center', va='bottom')
    plt.xlabel('Date',rotation=0)
    plt.savefig('data/figures/plot_SSR_sortie.png')
    plt.show()

def plot_SSR_entree(data_SSR, X):
    df_final=pd.DataFrame(data_SSR)
    df_final["date"]=X
    df_final=df_final.set_index("date")
    ...
    for p in ax.patches:
        ...
        ax.annotate(val, ((b.x0 + b.x1)/2 , int(b.y1)), ha='center', va='bottom')
    plt.xlabel('Date',rotation=0)
    plt.savefig('data/figures/plot_SSR_entree.png')
    plt.show()


plot_SSR(data_SSR, X)
plot_SSR_entree(data_entree, X)
plot_SSR_sortie(data_sortie, X)
